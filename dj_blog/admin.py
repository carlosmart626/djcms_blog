...
for lang in settings.LANGUAGES:
    ...
    def get_tag():
        ...
        def lang_tag(self, obj):
            ...
            if lang_object:
                return '<a href="/admin/dj_blog/posttitle/{}/" class="button">{} {}</a>'.format(
                    lang_object.id, view_string, lang_code.upper())
            return '<a class="button" href="/admin/dj_blog/posttitle/add/">{} {}</a>'.format(
...
class PostTitleAdmin(admin.ModelAdmin):
    ...
    def view_on_site(self, obj):
        if obj.published:
            return reverse('post-detail', kwargs={'blog_slug': obj.post.blog.slug, 'post_slug': obj.post.slug})
        else:
            return reverse('draft-post-detail', kwargs={'blog_slug': obj.post.blog.slug, 'post_slug': obj.post.slug})
...
for lang in settings.LANGUAGES:
    ...
    def get_tag():
        ...
        def lang_tag(self, obj):
            lang_object = obj.get_language_object(lang_code)
            if lang_object:
                return '<a href="/admin/dj_blog/blogtitle/{}/" class="button">{} {}</a>'.format(
                    lang_object.id, view_string, lang_code.upper())
            return '<a class="button" href="/admin/dj_blog/blogtitle/add/">{} {}</a>'.format(
                add_string, lang_code.upper())
        lang_tag.short_description = lang_code.upper()
        lang_tag.allow_tags = True
        lang_tag.__name__ = lang_code
...
class TagAdmin(admin.ModelAdmin):
    ...
    def view_on_site(self, obj):
        return reverse('tag-main', kwargs={'blog_slug': obj.blog.slug, 'tag_slug': obj.slug})
...
for lang in settings.LANGUAGES:
    ...
    def get_tag():
        ...
        def lang_tag(self, obj):
            lang_object = obj.get_language_object(lang_code)
            if lang_object:
                return '<a href="/admin/dj_blog/authorbio/{}/" class="button">{} {}</a>'.format(
                    lang_object.id, view_string, lang_code.upper())
            return '<a class="button" href="/admin/dj_blog/tagtitle/add/">{} {}</a>'.format(
                add_string, lang_code.upper())
        lang_tag.short_description = lang_code.upper()
        lang_tag.allow_tags = True
        lang_tag.__name__ = lang_code
...
class TagTitleAdmin(admin.ModelAdmin):
    ...
    def view_on_site(self, obj):
        return reverse('tag-main', kwargs={'blog_slug': obj.tag.blog.slug, 'tag_slug': obj.tag.slug})
