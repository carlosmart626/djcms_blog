...
def expire_blog_post(post):
    post_url = reverse('post-detail', kwargs={'blog_slug': post.post.blog.slug, 'post_slug': post.post.slug})
...
class AuthorBio(models.Model):
    ...
    language = models.CharField(max_length=15, db_index=True, choices=settings.LANGUAGES)
...
class Blog(models.Model):
    ...
    def get_language_object(self, language):
        language_object = BlogTitle.objects.filter(
            blog=self,
            language=language).first()
        if language_object:
            return language_object
...
class Tag(models.Model):
    ...
    def get_language_object(self, language):
        language_object = TagTitle.objects.filter(
            tag=self,
            language=language).first()
        if language_object:
            return language_object
...
class PostManager(models.Manager):
    ...
    def published(self):
        posts = PostTitle.objects.filter(published=True, publisher_is_draft=False).values_list('post', flat=True)
